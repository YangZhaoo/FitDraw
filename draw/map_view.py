...
class GlobalMap(ViewBase):
    ...
    def __init__(self):
        ...
        self._map = np.zeros((self._map_size, self._map_size, 3),
                             dtype=np.uint8)
        self._map_inner_delta_position = np.array([self._map_edge_size, self._map_edge_size])
    ...
    def _prepare_data(self, image, records: List[Record]):
        ...
        # 小地图的位置 右上角
        self._map_out_delta_position = np.array([self._image_w - self._map_size - 100, 100])
    ...
    def _draw(self, record: Record, image, **kargs):
        ...
        # cv.rectangle(image, self._map_out_delta_position + self._map_inner_delta_position,
        ...
        if record.location is None:
            return image
        current_point = self._convert_position(record.location.getNpArray())[0] + self._map_out_delta_position
